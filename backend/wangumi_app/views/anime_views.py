...
from wangumi_app.utils import build_error_response, resolve_cover_url
...
def _build_anime_list_response(request, base_queryset):
    ...
    if raw_sort:
        ...
        if order_by is None:
            return build_error_response("sort 参数仅支持 热度/时间/评分")
    else:
        order_by = "-popularity"
...
class AnimeListCreateView(APIView):
    ...
    def post(self, request):
        ...
        if not title:
            return build_error_response("必填项未填写完整：标题不能为空", status=400)
        ...
        try:
            ...
            return build_error_response("total_episodes 必须是整数", status=400)
        ...
        if release_date:
            try:
                ...
                return build_error_response("release_date 格式应为 YYYY-MM-DD", status=400)
        if cover:
...
@csrf_exempt
def anime_modify(request):
    raw_body = request.body.decode('utf-8')
    data = json.loads(raw_body) if raw_body else {}

    anime_id=data.get('item_id')
    anime = Anime.objects.filter(id=anime_id).first()
    if not anime:
        return JsonResponse({"code": 404, "message": "条目不存在", "data": None}, status=404)

    user = authenticate_user(request)
    if user is None:
        return build_error_response("需要登录才能修改条目", status=401)

    is_admin_user = bool(getattr(user, "is_staff", False) or getattr(user, "is_superuser", False))
    if anime.is_admin and not is_admin_user:
        return build_error_response("仅管理员可以修改官方条目", status=403)
    if not is_admin_user and anime.created_by_id != user.id:
        return build_error_response("无权限修改该条目", status=403)

    # 提取可修改字段
    title = (data.get('title') or '').strip()
    description = (data.get('description') or '').strip()
    cover_url = (data.get('cover_url') or data.get('cover') or'').strip()


    # 检查是否有任何修改内容
    has_modifications = any([title, description,cover_url])
    if not has_modifications:
        return build_error_response("没有提供任何修改内容", status=400)

    # 执行修改
    modified_fields = []
    if title:
        anime.title = title
        modified_fields.append("标题")
    if description:
        anime.description = description
        modified_fields.append("简介")
    if cover_url:
        anime.cover_url = cover_url
        modified_fields.append("封面图片")
...
def anime_delete(request, anime_id: int):
    ...
    user = authenticate_user(request)
    if user is None:
        return build_error_response("需要登录才能删除条目", status=401)
    is_admin_user = bool(getattr(user, "is_staff", False) or getattr(user, "is_superuser", False))
    if anime.is_admin and not is_admin_user:
        return build_error_response("仅管理员可以删除官方条目", status=403)
    if not is_admin_user and anime.created_by_id != user.id:
        return build_error_response("无权限删除该条目", status=403)
    anime.delete()
    return JsonResponse(
...
def anime_detail(request, anime_id: int):
    ...
    episodes_payload = []
    for ep in Episode.objects.filter(anime=anime).order_by("episode_number"):
        online_urls = []
        raw_urls = ep.online_urls
        if raw_urls:
            try:
                import json

                parsed = json.loads(raw_urls) if isinstance(raw_urls, str) else raw_urls
    ...
    data = {
        ...
        "meta": {
            ...
            "episodes_detail": episodes_payload,
